...
#else
      if (!(sit->sev & not_sev) && p_LmDivisibleBy(sit->p, p, r))
#endif
      {
...
sBasisSet::iterator kFindDivisibleByInS(const kStrategy strat, LObject* L)
{
  ...
  auto ende_it = strat->S.find_divisor_search_bound(p, strat->S.end(), strat);
...
sBasisSet::iterator kFindDivisibleByInS_noCF(const kStrategy strat, LObject* L)
{
  unsigned long not_sev = ~L->sev;
  poly p = L->GetLmCurrRing();
...
sBasisSet::iterator kFindNextDivisibleByInS(const kStrategy strat, sBasisSet::iterator start, LObject* L)
{
  ...
  pAssume(~not_sev == p_GetShortExpVector(p, currRing));
  for (auto sit = start; sit!=strat->S.end(); ++sit)
  {
#if defined(PDEBUG) || defined(PDIV_DEBUG)
    if (p_LmShortDivisibleBy(sit->p, sit->sev,
                             p, not_sev, currRing))
...
// TODO warum SetpFDeg notwendig?
  ...
  {
    /* check if a reducer of the lead term exists */
    sit_red = kFindDivisibleByInS(strat, h);
...
static int redRing_S (LObject* h,kStrategy strat)
{
  ...
  loop
  {
    sit = kFindDivisibleByInS(strat, h);
    if (sit == strat->S.end())
    {
      // over ZZ: cleanup coefficients by complete reduction with monomials
      postReduceByMon(h, strat);
...
//  };
  ...
  loop
  {
    auto sit_j = kFindDivisibleByInS_noCF(strat,&P);
    auto sit_j_ring = sit_j;
    while ((sit_j != strat->S.end())
    && (nonorm)
    && (!n_DivBy(pGetCoeff(P.p),pGetCoeff(sit_j->p),currRing->cf)))
    {
      auto next = sit_j; ++next;
      sit_j = kFindNextDivisibleByInS(strat,next,&P);
    }
    if (sit_j != strat->S.end())
    {
      ...
      {
        int sll;
        { auto next = sit_jj; ++next;
          sit_jj = kFindNextDivisibleByInS(strat,next,&P); }
        if (sit_jj == strat->S.end()) break;
        if ((!nonorm)
        || (n_DivBy(pGetCoeff(P.p),pGetCoeff(sit_jj->p),currRing->cf)))
        {
          sll=pSize(sit_jj->p);
          if (sll<sl)
          {
            #ifdef KDEBUG
            if (TEST_OPT_DEBUG) Print("better(S%d:%d -> S%d:%d)\n",sit_j.index(),sl,sit_jj.index(),sll);
            #endif
...
#endif
      {
        kBucketPolyRedNF(P.bucket,sit_j->p,pLength(sit_j->p),
                            strat->kNoether);
      }
      cnt--;
      if (cnt==0)
    ...
    else if (is_ring && (sit_j_ring != strat->S.end()) && (currRing->cf->cfQuotRem!=ndQuotRem))
    {
      number r;
      auto sit_jr = sit_j_ring;
      number n=n_QuotRem(pGetCoeff(P.p),pGetCoeff(sit_jr->p),&r,currRing->cf);
      if(!n_IsZero(n,currRing->cf))
      {
        poly lm=kBucketGetLm(P.bucket);
        poly m=p_Head(lm,currRing);
        p_ExpVectorSub(m,sit_jr->p,currRing);
        if (p_GetComp(sit_jr->p, currRing) != p_GetComp(lm, currRing))
        {
...
#endif
        int l=-1;
        kBucket_Minus_m_Mult_p(P.bucket,m,sit_jr->p,&l);
        P.p=kBucketGetLm(P.bucket);
        p_Delete(&m,currRing);
#ifdef KDEBUG
        if (TEST_OPT_DEBUG)
...
poly redNFBound (poly h,int nonorm,kStrategy strat,int bound)
{
  h = pJet(h,bound);
  if (h==NULL) return NULL;
  ...
  loop
  {
    sit_j=kFindDivisibleByInS(strat, &P);
    if (sit_j != strat->S.end())
    {
      if (!is_ring)
      {
...
ideal bba (ideal F, ideal Q,intvec *w,bigintmat *hilb,kStrategy strat)
{
  ...
  // rr-replay hardware watchpoint, 20 Apr 2026.
  {
    ...
    if(!__will_go_parallel && !idIs0(F) &&(!rField_is_Ring(currRing)))  // create strong gcd poly computes with tailring and S[i] ->to be fixed
...
bba_post_loop:
  ...
  if (!errorreported)
  {
    if(rField_is_Z(currRing))
    {
      for(auto sit = strat->S.begin(); sit != strat->S.end(); ++sit)
      {
        if(!nGreaterZero(pGetCoeff(sit->p)))
        {
          sit->p = pNeg(sit->p);
        }
      }
      finalReduceByMon(strat);
      // finalReduceByMon may NULL entries (poly reduced to zero); skip those
      for(auto sit = strat->S.begin(); sit != strat->S.end(); ++sit)
      {
        if(sit->p != NULL && !nGreaterZero(pGetCoeff(sit->p)))
        {
          sit->p = pNeg(sit->p);
        }
      }
...
//  }
...
#ifdef KDEBUG
  for(auto _sit = strat->S.begin(); _sit != strat->S.end(); ++_sit)
  ...
  if ((Q!=NULL)&&(!errorreported)) updateResult(Q,strat);
#ifdef KDEBUG
  for(auto _sit = strat->S.begin(); _sit != strat->S.end(); ++_sit)
    if (_sit->p != NULL && _sit->p->coef == NULL)
...
ideal sba (ideal F0, ideal Q,intvec *w,bigintmat *hilb,kStrategy strat)
{
  ...
  initSba(F,strat);
  /*set enterS, spSpolyShort, reduce, red, initEcart, initEcartPair*/
  initSbaBuchMora(F, Q,strat);
  if (strat->minim>0) strat->M=idInit(IDELEMS(F),F->rank);
  srmax = strat->S.size()-1;
  reduc = olddeg = 0;
#ifndef NO_BUCKETS
  if (!TEST_OPT_NOT_BUCKETS)
    strat->use_buckets = 1;
...
//#if 1
    ...
    if (strat->sbaOrder == 1)
    {
      for (int jj = 0; jj<strat->T.size(); jj++)
      {
    ...
    else
    {
      ...
      //if ((!TEST_OPT_IDLIFT) || (pGetComp(strat->P.p) <= strat->syzComp))
      enterT(strat->P, strat);
      strat->T[strat->T.size()-1].is_sigsafe = FALSE;
      /*
      printf("hier\n");
      pWrite(strat->P.GetLmCurrRing());
      pWrite(strat->P.sig);
      */
      ...
      if(strat->sbaOrder != 1)
      {
        ...
        for (auto sit_tk=strat->S.begin(); sit_tk!=strat->S.end(); ++sit_tk)
        {
          if (pGetComp(sit_tk->sig) == pGetComp(strat->P.sig))
          {
            //printf("TK %d / %d\n",tk,strat->S.size()-1);
            overwrite = FALSE;
            break;
        ...
        if (overwrite)
        {
          ...
          int i;
          LObject Q;
          for(auto sit_ps=strat->S.begin(); sit_ps!=strat->S.end(); ++sit_ps)
          {
            ...
            if (strat->sbaOrder == 0)
              p_ExpVectorAdd (Q.sig,sit_ps->p,currRing);
            ...
            poly help = p_Copy(sit_ps->sig,currRing);
      ...
      if(strat->sbaOrder == 0 || strat->sbaOrder == 3)
      {
        ...
        {
          {
          int last_idx = strat->S.size()-1; // exclude the element just added
          for (auto sit_i=strat->S.begin(); sit_i!=strat->S.end() && sit_i.index() < last_idx; ++sit_i)
          {
            Q.sig = p_Copy(strat->P.sig,currRing);
            p_ExpVectorAdd(Q.sig,sit_i->p,currRing);
            poly help = p_Copy(sit_i->sig,currRing);
            p_ExpVectorAdd(help,strat->P.p,currRing);
            Q.sig = p_Add_q(Q.sig,help,currRing);
            //pWrite(Q.sig);
...
//      Print("[%d]",hilbeledeg);
      kDeleteLcm(&strat->P);
      if (strat->S.size()-1>srmax) srmax = strat->S.size()-1;
    }
    else
    {
...
//  }
  ...
  if(rField_is_Ring(currRing))
  {
    //for(int k = strat->S.size()-1;k>=0;k--)
    ...
    #endif
    //for(int kk = strat->S.size()-1;kk>=0;kk--)
...
//#define KSTD_NF_NOLF   4096
  ...
  p = redNF(pCopy(q),(lazyReduce & KSTD_NF_NONORM)==KSTD_NF_NONORM,strat);
  if ((p!=NULL)&&((lazyReduce & KSTD_NF_LAZY)==0))
  {
    ...
    else
    {
      ...
      p = redtailBba(p,strat->S.end(),strat,(lazyReduce & KSTD_NF_NONORM)==0);
    }
  }
  /*- release temp data------------------------------- -*/
  strat->S.free_all();
...
//#define KSTD_NF_NONORM 4
  ...
  if (TEST_OPT_PROT) { PrintS("r"); mflush(); }
  if (BVERBOSE(23)) kDebugPrint(strat);
  p = redNFBound(pCopy(q),lazyReduce & KSTD_NF_NONORM,strat,bound);
  if ((p!=NULL)&&((lazyReduce & KSTD_NF_LAZY)==0))
  {
    if (TEST_OPT_PROT) { PrintS("t"); mflush(); }
    if (rField_is_Z(currRing)||(rField_is_Zn(currRing)))
          ...
          intvec *w,bigintmat *hilb )
{
  ...
  hilbcount=0;
  minimcnt=0;
  srmax = 0; // strat->S.size()-1 is 0 at this point
  reduc = olddeg = 0;
  ...
  while ((!strat->T.empty()))
  {
    if(!strat->T[strat->T.size()-1].is_redundant)
    {
      LObject h;
      h.p = strat->T[strat->T.size()-1].p;
      h.tailRing = strat->T[strat->T.size()-1].tailRing;
      h.t_p = strat->T[strat->T.size()-1].t_p;
      if (h.p!=NULL)
      {
...
//#ifdef HAVE_TAIL_RING
  if(!rField_is_Ring())  // create strong gcd poly computes with tailring and S[i] ->to be fixed
    kStratInitChangeTailRing(strat);
#endif
  //enterpairs(pOne(),0,0,-1,strat,strat->T.size()-1);
  //strat->S.setsize(0);
      ...
      for (auto sit_k = strat->S.begin(); sit_k != strat->S.end(); )
      {
        ...
        for (int j = 0; j < strat->T.size(); ++j)
        {
          if (strat->T[j].p!=NULL)
          {
            ...
            assume(sit_k->sev == pGetShortExpVector(sit_k->p));
            if (pLmShortDivisibleBy(strat->T[j].p, strat->sevT[j], sit_k->p, ~sit_k->sev))
            {
