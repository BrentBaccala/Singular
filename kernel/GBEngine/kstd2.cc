    ...
    if (j < 0)
    {
      ...
      if(j < 0)
      {
        ...
        if(pLtCmp(beforeredsig,h->sig) == 1)
        {
          ...
          {
            //strat->enterS(*h, strat->S.size(), strat, strat->T.size()-1);
            return 0;
          }
        }
  ...
  loop
  {
    ...
    if (j>=0)
    {
      ...
      loop
      {
        ...
        if (jj<0) break;
        auto sit_jj = strat->S.iterator_at(jj);
        if ((!nonorm)
    ...
    {
      number r;
      auto sit_jr = strat->S.iterator_at(j_ring);
...
ideal sba (ideal F0, ideal Q,intvec *w,bigintmat *hilb,kStrategy strat)
{
  ...
  if(rField_is_Ring(currRing) && strat->sbaEnterS >= 0)
  {
    for(int i = 0;i<strat->sbaEnterS;i++)
    {
      ...
      strat->L.pop();
      enterT(Lp,strat);
      strat->enterS(Lp, strat->S.size(), strat, strat->T.size()-1);
    }
    strat->sbaEnterS = -1;
  }
  kTest_TS(strat);
...
*/
    ...
    if(rField_is_Ring(currRing) && strat->sigdrop)
    {
      ...
      {
        strat->enterS(strat->P, 0, strat, strat->T.size()-1);
        if (TEST_OPT_PROT)
          PrintS("-");
        break;
      }
...
//  }
  ...
  if(rField_is_Ring(currRing))
  {
    ...
    {
      //printf("\nAdded k = %i\n",k);
      strat->enterS(*it, strat->S.size(), strat, strat->T.size()-1);
